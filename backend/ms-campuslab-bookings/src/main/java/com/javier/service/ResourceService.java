...
public class ResourceService {
    ...
    public ResourceResponse updateResource(Long id, UpdateResourceRequest request) {
        ...
        if (request.getTotalStock() != null) {
            int previousAvailable = resource.getAvailableStock();
            int previousTotal = resource.getTotalStock();
            int delta = request.getTotalStock() - previousTotal;
            resource.setTotalStock(request.getTotalStock());
            resource.setAvailableStock(Math.max(0, previousAvailable + delta));
        }
